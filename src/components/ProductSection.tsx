...
export default async function ProductSection() {
  ...
  return (
    <section id="sorten" className="px-6 py-24" style={{ background: "var(--lh-cream)" }}>
      <div className="lh-container">
        <div className="lh-section-header mb-10">
          <p className="lh-eyebrow">Unsere Sorten</p>
        ...
        <div className="mb-12 max-w-2xl">
          ...
          <p
            className="mt-3"
            style={{ fontFamily: "var(--font-sans)", fontSize: "var(--fs-16)", color: "var(--lh-ink-3)" }}
          >
            Alle Sorten in 250 g und 500 g erhältlich.
          </p>
        </div>
        ...
        {products.length > 0 ? (
                  ...
                  <div className="mt-5 flex flex-grow flex-col">
                    ...
                    {product.description && (
                      <p
                        ...
                        style={{ fontFamily: "var(--font-sans)", fontSize: "var(--fs-14)", lineHeight: 1.55, color: "var(--lh-ink-3)" }}
                      >
                        {product.description}
                      </p>
                    )}
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {placeholders.map((name) => (
              <div key={name} className="lh-card flex h-full flex-col" style={{ padding: 16 }}>
                <div
                  className="flex aspect-[4/3] items-center justify-center"
                  style={{ background: "linear-gradient(160deg, #F5E6A8 0%, #C9A642 100%)", borderRadius: 10 }}
                >
                  <HoneyJarPlaceholder />
                </div>
                <div className="mt-5">
                  <h3 style={{ fontFamily: "var(--font-heading)", fontSize: "var(--fs-20)", fontWeight: 500, color: "var(--lh-ink)" }}>
                    {name}
                  </h3>
        ...
        <div className="mt-12 flex justify-start">
          <Link href="/honig" className="lh-btn lh-btn--primary lh-btn--lg">
            <ShoppingBasket size={16} /> Alle Sorten im Shop
          </Link>
        </div>
      </div>
    </section>
